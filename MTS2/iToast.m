...
- (void) show:(iToastType) type{
	...
	UILabel *label = [[UILabel alloc] initWithFrame:CGRectIntegral(CGRectMake(0, 0, textSize.width + 5, textSize.height + 5))];
	...
	if (image) {
		v.frame = CGRectIntegral(CGRectMake(0, 0, image.size.width + textSize.width + 15, MAX(textSize.height, image.size.height) + 10));
		label.center = CGPointMake(image.size.width + 10 + (v.frame.size.width - image.size.width - 10) / 2, v.frame.size.height / 2);
	} else {
		v.frame = CGRectIntegral(CGRectMake(0, 0, textSize.width + 10, textSize.height + 10));
	...
	if (image) {
		...
		imageView.frame = CGRectIntegral(CGRectMake(5, (v.frame.size.height - image.size.height)/2, image.size.width, image.size.height));
